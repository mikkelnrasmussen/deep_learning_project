...
discriminator_model = '/work3/s193518/deep-learning-project-03/code/results_plus_VAE_1_with_discrim/discriminator_final.model'
...
modelDiscriminator = torch.load(discriminator_model)
modelDiscriminator = modelDiscriminator.to('cpu')
...
loss_elbo, xhat, diagnostics, outputs = vi(modelVAEplus, x)

# Pass x-hat (reconstructions) through Discriminator
# without calculating gradients
output_hat, inter_repr_fake = modelDiscriminator(xhat)

# Pass real images from batch 1 (b1) through Discriminator
# without calculating gradients
output_real_b1, inter_repr_real = modelDiscriminator(x)

loss_repr_func = nn.MSELoss()
#ch1 = random.randint(1, 64)
#ch2 = random.randint(1, 64)
#ch3 = random.randint(1, 64)

for i in range(10):
    fig, axes = plt.subplots(2, 5, figsize=(9, 4), squeeze=False)

    axes[0, 0].set_title('Original image')
    axes[1, 0].set_title('Reconstruction')
    axes[0, 1].set_title('Layer 1')
    axes[0, 2].set_title('Layer 2')
    axes[0, 3].set_title('Layer 3')
    axes[0, 4].set_title('Layer 4')

    x_img = x[i].to('cpu')
    x_img = make_grid(x_img, nrow=0).permute(1, 2, 0)

    xhat_img = xhat[i].to('cpu')
    xhat_img = make_grid(xhat_img, nrow=0).permute(1, 2, 0)

    axes[0, 0].imshow(x_img.detach().numpy())
    axes[1, 0].imshow(xhat_img.detach().numpy())

    axes[0, 0].axis('off')
    axes[1, 0].axis('off')


    for j, (repr_fake, repr_real) in enumerate(zip(inter_repr_fake, inter_repr_real)):
        print(f"Layer: {j}")
        repr_real = repr_real[i].to('cpu')
        repr_real = repr_real[0:3, :, :]
        repr_real = make_grid(repr_real, nrow=1).permute(1, 2, 0)

        repr_fake = repr_fake[i].to('cpu')
        repr_fake = repr_fake[0:3, :, :]
        repr_fake = make_grid(repr_fake, nrow=1).permute(1, 2, 0)
        
        axes[0, j+1].imshow(repr_real.detach().numpy())
        axes[1, j+1].imshow(repr_fake.detach().numpy())

        axes[0, j+1].axis('off')
        axes[1, j+1].axis('off')

    fig.savefig(result_dir + 'intermediate_representations_' + str(i) + '.png', bbox_inches='tight')
    plt.close()

for i in range(10):

    x_real_img = x[i].to('cpu').permute(1, 2, 0)
    plt.imshow(x_real_img.detach().numpy())
    plt.axis('off')
    plt.savefig(result_dir + 'real_image' + str(i))

    xhat_fake_img = xhat[i].to('cpu').permute(1, 2, 0)
    plt.imshow(xhat_fake_img.detach().numpy())
    plt.axis('off')
    plt.savefig(result_dir + 'reconstruction_image' + str(i) + '.png')
    plt.close()
